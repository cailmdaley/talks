  ...
  <div class="methods">
    ...
    <div class="mrow2 apart fragment">
      <div class="mtxt"><div class="mbox">
        ...
        <span class="eq">\( {\rm Cov}(\xi_i,\xi_j)=\big(N_{{\rm pair},i}N_{{\rm pair},j}\big)^{-1}\textstyle\sum_{abcd} w_aw_bw_cw_d\,\Delta_i(\theta_{ab})\,\Delta_j(\theta_{cd}) \)</span>
        <span class="eq">\( \qquad\qquad\times\;\tfrac12\big[\xi(\theta_{ac})\xi(\theta_{bd})+\xi(\theta_{ad})\xi(\theta_{bc})\big] \)</span>
        </div>
      </div>
...
<div class="nka" data-step="10" data-staged="1">
  <div class="corr big">
    <div>
      <span class="nm">quartic counts (Schneider; Shirasaki)</span>
      <span class="eq">\( {\rm Cov}(\xi_i,\xi_j)=\big(N_{{\rm pair},i}N_{{\rm pair},j}\big)^{-1}\textstyle\sum_{abcd} w_aw_bw_cw_d\,\Delta_i(\theta_{ab})\,\Delta_j(\theta_{cd}) \)</span>
        <span class="eq">\( \qquad\qquad\times\;\tfrac12\big[\xi(\theta_{ac})\xi(\theta_{bd})+\xi(\theta_{ad})\xi(\theta_{bc})\big] \)<span class="tag">\( \mathcal{O}(N_{{\rm pair},i}N_{{\rm pair},j}) \) per element</span></span>
    </div>
  </div>
  <span class="fragment nka-step"></span>
  <span class="fragment nka-step"></span>
...
Cost, since the badge is on the slide. Naively this is the number of pairs in bin i times the
number in bin j, for every element — order N to the fourth in the sources. Their contribution is
a matrix rewrite that turns it into BLAS-3 products over pair-indexed vectors, plus coarse-graining
