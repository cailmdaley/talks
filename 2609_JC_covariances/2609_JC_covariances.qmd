...
<div class="nka takeaways">
  <div class="tk">
    ...
    <ul>
      <li>Band-limit it at the interparticle scale; treat self-pairs exactly. The NKA carries only the distinct pairs.</li>
      <li>Exact where sparse, NKA where dense &mdash; in NaMaster v3.</li>
    </ul>
  </div>
  <div class="tk">
    <span class="nm">Nagura et al. &mdash; diagonal accuracy is not enough</span>
    <ul>
      <li>iNKA is right on the \( \ell \) diagonal (\( \lesssim 10\% \)) and wrong off it; the Legendre transform sums every off-diagonal \( \rightarrow \) &minus;25&ndash;30% on \( {\rm Cov}(\xi,\xi) \).</li>
      <li>Count the pairs instead of transforming an approximated matrix.</li>
    </ul>
  </div>
</div>
```
