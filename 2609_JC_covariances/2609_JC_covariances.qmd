...
::: {.notes}
Same field. Each wave below is one Fourier mode of it — one multipole, one amplitude, one
phase. Click through them, low ell first.

Each one also puts a point on the spectrum underneath: the expected squared amplitude at that
multipole, sitting right on the true curve. Watch the fourth one land above the third — that's
...
<span class="fragment nka-step"></span>
<svg class="pA" data-h="170" viewBox="0 0 1730 260" width="1730" height="260"></svg>
<svg class="pC" data-h="230" data-kernel="1" data-kernel-at="1" data-band-at="2" data-kh="148"
     data-tilde="1" data-tilde-at="3"
...
evaluate it once at the midpoint of the two modes — C-bar, the average of C at ell and at ell-prime
— and pull it outside the sum. What is left collapses into a single kernel, and the cost
drops to the same order as the estimator itself. And notice what that kernel is: it is exactly the
coupling kernel from two slides ago, the mask's own power at mode separation ell minus ell-prime
...
C-ac and C-bd, and the kernel takes two mask products, w-a w-c and w-b w-d, plus the term with c and d
...
C to be constant. C-bar only has to be a good stand-in for C wherever the kernel is not already zero —
...
<div class="two lower">
  ...
  <div class="figblock fragment" data-fragment-index="1">
    ...
    <div class="figside">
      ...
      <p class="verbatim caption">Mask pseudo power spectrum for the <i>Quaia</i> survey mask, comparing catalogue- and mask-based estimators. At the catalogue level, the constant shot noise floor (\(\tilde{N}_w\)) is reached at scales far below the mean interparticle scale (\(\theta_{\rm c}\)). For the catalogue estimates, we Poisson-sample \(10^5\) positions with number densities following the survey mask and compute the spherical harmonics at the catalogue level.</p>
    </div>
  </div>
</div>
</div>
...
left. Uniform random positions, and I am just summing e-to-the-minus-i-ell-x over the sources I
drew. It is flat. At every ell the phases of a hundred and forty deltas point in random
...
exist; there is nothing else to remove. Watch the low ell. Below the inverse footprint scale the
...
footprint, so the spectrum lifts off the floor — and by ell of about fifteen it has fallen back onto the floor
...
precision. A delta function has power at every ell, all the way up, forever. So stop pretending:
...
is a multiplication by the cloud's transform, e to the minus ell squared theta-c squared — and
watch what it does. The coherent part at low ell, which is the actual footprint, is untouched. The
...
So drag until the red lies on the blue for as far in ell as you can get it.

Two scales, and they are not the same. The Gaussian is down to one over e at ell equals one over
...
<div class="splitfig">
  <svg class="pSplit" data-h="336" data-x0="104" data-w="1520"
       viewBox="0 0 1730 446" width="1730" height="446"></svg>
</div>

<div class="corr big tight">
  <div class="fragment" data-fragment-index="2">
    <span class="nm noise">self-pair field</span>
    <span class="eq">\( n(x)\;\equiv\;\textstyle\sum_i w_i^{2}\,a_i^{2}\,G^{2}_{\theta_{\rm c}}(x-x_i),\qquad N_{\ell-\ell'}=\textstyle\sum_x n(x)\,e^{-i(\ell-\ell')x} \)</span>
  </div>
  <div class="fragment" data-fragment-index="2">
    <span class="nm signal">distinct-pair mask product</span>
    <span class="eq">\( \mathcal{W}(x)\;\equiv\;\bar w^{2}(x)\;-\;\textstyle\sum_i w_i^{2}\,G^{2}_{\theta_{\rm c}}(x-x_i) \)</span>
  </div>
...
::: {.notes}
The mask is smooth now. So write down what the covariance actually needs, and let it tell us
what the next problem is.

Start from the catalogue modes on the earlier slide — a-tilde-ell is a sum over sources, weight
times value times a phase. Take the two-point function of that. You get a double sum over
sources, and there is exactly one thing you can do with a double sum over a discrete set: split
it by whether the two sources are the same source or not.

That split is the whole slide, and every line under it is forced by it.

Distinct pairs, i not equal to j, carry the correlation between two different points — that is
the signal, and it is mask-coupled in the ordinary way. Self-pairs, i equal to i, carry the
square of the field at a single point. And look at the phase on that second term: it depends only
on ell minus ell-prime. Not on ell and ell-prime separately. That is the algebraic signature of a
statistically homogeneous noise field, and it does not go away when you smooth the mask. A spike
at zero lag is a flat line at every multipole, so the self-pair term behaves exactly like white
noise — and white noise is the one thing this machinery already handles exactly.

The picture is the same split, in position space. Blue is the band-limited mask from the previous
slide. Square it, because the covariance kernel wants the mask product, and the square separates
into two stacked bands — the thin blue outline is the mask itself, still there underneath. The red
floor is every source paired with itself; the blue bulk above it is every distinct pair. The red
band tracks the source density, it is largest where sources are sparse relative to the cloud size,
and it is there for a perfectly noiseless field.

One thing about the red band: it is the self-pair part of the MASK product, sum of w-i squared
times the squared cloud. The local variance map on the next line is that same band weighted by
the data, a-i squared source by source. Same geometry, different weight.

So, first line underneath. Name the self-pair piece. It is a local variance map — literally the
data squared, source by source, smeared by the squared cloud — and its transform is the N that
appears in the covariance. That is the connection: the noise in the covariance is the harmonic
transform of that red band.

A footnote I will not put on the slide. That map wants the *expected* square of the field at each
source, and all you have is the one value you measured, so the paper simply uses it — a-i squared
in place of its expectation. That is exact on average and noisy per source, which is fine until
you take its power spectrum: squaring a squared quantity brings in fourth moments, and since the
expectation of a product is not the product of expectations, the variance map's power comes out
too high at large ell. They subtract that analytically with Wick — a self-pair correction to the
self-pair term.

Second line: take the self-pairs out of the mask product, or they get counted twice. What is left
is exactly the distinct-pair sum. Note that the subtracted piece is its own catalogue field, built
from the transform of the squared cloud, not the square of the transform — squaring a Gaussian
halves its width.

Third line, and this is the payoff: the narrow-kernel approximation from the first half of the
talk, applied to the distinct-pair product and to nothing else. Signal against signal, signal
against noise, noise against noise. Only the first carries the approximation. Everything with a
self-pair in it is exact.
...
## Implemented in NaMaster v3 {.nka-slide}
...
needs — the C-bars from the covariance slide.
...
<div class="nka" data-step="10">
  <div class="methods">
    <div class="mhead">
      <div class="mtxt"><span class="eunit">numbers beside \( \Delta \): peak &plusmn;, in units of the exact diagonal</span></div>
    ...
    <div class="mrow2 apart fragment">
      <div class="mtxt"><div class="mbox">
        ...
        <span class="eq">\( {\rm Cov}(\xi_i,\xi_j)=\displaystyle\sum_{ab\,\in\,i}\;\sum_{cd\,\in\,j}\;\frac{w_aw_bw_cw_d}{N_{{\rm pair},i}N_{{\rm pair},j}}\;\cdot\;\tfrac12\big[\xi(\theta_{ac})\xi(\theta_{bd})+\xi(\theta_{ad})\xi(\theta_{bc})\big] \)</span>
        </div>
      </div>
...
Hold on to the middle column. The next three slides are all about what those differences do
to a real error bar.
:::

## Off-diagonal errors are amplified by the Legendre transform {.nka-slide}

```{=html}
<div class="nka">
<div class="r-stack figswap">
  <div class="figone fragment fade-out" data-fragment-index="0">
    <img src="images/nagura_matrix_r.png" alt="Correlation matrices of the harmonic covariance: iNKA, simulations, and their ratio">
    <div class="papercap"><span class="src">Nagura et al., arXiv:2605.16186 &mdash; Fig. 5</span>Correlation matrices of \(\mathrm{Cov}(\tilde C_\ell,\tilde C_{\ell'})\) computed using the iNKA (left) and measured from Gaussian simulations (center). The colors corresponding to the map values are depicted on the left. The right plot shows the fractional difference \(\mathrm{Cov}(\tilde C_\ell,\tilde C_{\ell'})^{\rm iNKA}/\mathrm{Cov}(\tilde C_\ell,\tilde C_{\ell'})^{\rm sim}-1\) and color bar is shown on the right. The iNKA reproduces the diagonal structure accurately, while noticeable differences remain in the off-diagonal components.</div>
  </div>
  <div class="figone withbox fragment fade-in" data-fragment-index="0">
    <div class="figbox">
    <div class="punchbox">
      <span class="nm">\( {\rm Cov}^{\rm iNKA}/{\rm Cov}^{\rm sim}-1 \), HSC-Y3 mask</span>
      <span class="pq">on \( {\rm diag}\,{\rm Cov}(\tilde C_\ell,\tilde C_\ell) \)</span>
      <span class="pv">within 10% &nbsp;<i class="pvs">Fig. 3</i></span>
      <span class="par">&darr;</span>
      <span class="pq">on \( {\rm diag}\,{\rm Cov}(\xi,\xi) \), \( \theta\approx6' \)</span>
      <span class="pv hot">&minus;25 to &minus;30% &nbsp;<i class="pvs">Fig. 4</i></span>
    </div>
    <div class="caption boxnote">HSC-Y3: six disjoint patches &mdash; the hardest geometry for a narrow kernel.</div>
    </div>
    <img src="images/nagura_cov_xi.png" alt="Cov(xi,xi) diagonal: sims, projected sim harmonic covariance, projected iNKA">
    <div class="papercap"><span class="src">Nagura et al., arXiv:2605.16186 &mdash; Fig. 4</span>Diagonal components of \(\mathrm{Cov}(\xi[\theta_-,\theta_+],\xi[\theta'_-,\theta'_+])\), measured from Gaussian realizations (dashed line) and calculated using Eq. 2.8 (solid lines).</div>
  </div>
</div>
</div>
```
...
::: {.notes}
Two figures on this slide, in the paper's order: first their diagnosis, then the result.

The diagnosis. iNKA's correlation matrix on the left, the simulated one in the
...
what you would get from signal much smaller than noise with random sign. The diagnosis is
argued by elimination. The obvious clean test — keep only the first N off-diagonal bands
and project — is not in the paper. I ran it in the toy; it comes back at the end.

Click. The result, which I believe completely: the controlled experiment, and it is the
cleanest thing in the paper. Black dashed is the covariance of xi measured directly from a
thousand simulations. Blue is the *simulated harmonic* covariance pushed through that
double Legendre sum. Blue lands on black — so the transform, the pair-count normalisation,
and the ell-max of six thousand are all fine. Red is iNKA through the same machinery, and
it sits visibly low: twenty-five to thirty percent at six arcminutes, shrinking to nothing
by a couple of hundred. The failure is isolated to the covariance model. Nothing else in
the chain is at fault. The box is the paper in two numbers: within ten percent on the
harmonic diagonal, thirty percent low in real space.
:::

## Pairs of pairs {.nka-slide}

```{=html}
<div class="nka" data-step="10" data-staged="1">
  <div class="corr big">
    <div>
      <span class="nm">quartic counts (Schneider; Shirasaki)</span>
      <span class="eq">\( {\rm Cov}(\xi_i,\xi_j)=\displaystyle\sum_{ab\,\in\,i}\;\sum_{cd\,\in\,j}\;\frac{w_aw_bw_cw_d}{N_{{\rm pair},i}N_{{\rm pair},j}}\;\cdot\;\tfrac12\big[\xi(\theta_{ac})\xi(\theta_{bd})+\xi(\theta_{ad})\xi(\theta_{bc})\big] \)<span class="tag">\( \mathcal{O}(N_{{\rm pair},i}N_{{\rm pair},j}) \) per element</span></span>
    </div>
  </div>
  <span class="fragment nka-step"></span>
  <span class="fragment nka-step"></span>
  ...
  <span class="fragment nka-step"></span>
  <span class="fragment nka-step"></span>
  <svg class="pQuad" viewBox="0 0 1730 700" width="1730" height="700"></svg>
</div>
```

::: {.notes}
This is the estimator, drawn. Same catalogue as the pair-count slide, same field.

A correlation function bin is a set of pairs. So a *covariance* between two bins is a sum over
pairs of pairs: one pair from bin i, one pair from bin j.

Here is a pair at theta-i — two sources, that separation, in red. And here is a pair at theta-j,
in blue. Four objects, a b c d.

Now the Gaussian four-point function. It factorises into two cross-pairings, and these are the
four cross-separations: a to c with b to d, or a to d with b to c. Each pairing contributes the
product of the theory xi at those two separations. Notice which separations those are — not
theta-i and theta-j, but the separations *between* the two pairs. That is where the geometry
enters, and it enters as an actual distance between actual objects.

Now do that for every pair in bin i against every pair in bin j — a couple of dozen against a couple of
dozen here, the label counts them, and that is one cell of the matrix. Fill in every cell and you have the whole
covariance.

And here is the thing worth saying slowly: nowhere in that did a window function appear. No
coupling matrix, no f-sky, no assumption about a kernel width. The footprint, the holes, the
boundaries and the weights are all encoded exactly by which quadruplets exist to be counted. The
window is not modelled. It is counted. That is the entire idea, and the matrix on the right is
the one you saw in the last row of the previous slide.

Cost, since the badge is on the slide, and it is worth doing out loud. One element of this matrix
is a sum over every pair in bin i against every pair in bin j — N-pair-i times N-pair-j terms. The
number of pairs goes like the number of galaxies squared. So one element costs galaxies to the
fourth. That is where the name comes from: quartic, in the sources. Their contribution is a matrix
rewrite that turns it into BLAS-3 products over pair-indexed vectors, plus coarse-graining
the galaxies onto an N-side of two fifty-six or five twelve depending on the angular bin. Worth
saying out loud: they report no wall-clock time, no scaling law and no memory footprint anywhere,
so the practical case for the method rests on a cost they never state. And the coarse-graining is
the same band-limiting move as Wolz's clouds, chosen for the same reason and with the same
arbitrariness.
:::

## Accounting for geometry with counts {.nka-slide}
...
<div class="figpair">
  ...
  <div class="papercap"><span class="src">Nagura et al., arXiv:2605.16186 — Fig. 6</span>Diagonal components of \(\mathrm{Cov}(\xi[\theta_-,\theta_+],\xi[\theta'_-,\theta'_+])\), measured from Gaussian realizations (dashed line) and calculated using analytical methods (solid lines). Note that we consider XMM field only for this figure.</div>
</div>
</div>
```
...
And the scope of this figure specifically: the XMM field alone, one of six, with ten bins instead
of twenty, to keep the cost down. So the best method is never tested on the full footprint the
failure was diagnosed on — the two key figures are not on the same configuration.
...
```{=html}
<div class="nka morsels">
  <div class="mcol">
    <span class="who">Wolz et al.</span>
    <blockquote>&ldquo;Eq. 24 is strongly reminiscent of the expression for the two-point correlator of a field with white noise, &hellip; with the self-pairs contribution playing the role of noise, and the sum over distinct pairs that of the signal.&rdquo;<span class="cite">&sect; Auto-correlations</span></blockquote>
    <blockquote>&ldquo;the noise-like contribution from self-pairs is an intrinsic aspect of fields sampled at discrete points that penalises sparser samples with larger statistical uncertainties.&rdquo;<span class="cite">&sect; Auto-correlations</span></blockquote>
    <blockquote>&ldquo;Choosing a finite value for \( \ell_{\rm max} \) &hellip; is equivalent to band-limiting the harmonic transform of the mask or, in other words, treating each source not as a point but as a &lsquo;cloud&rsquo; of finite size \( \theta_{\rm c} \).&rdquo;<span class="cite">&sect; Disjoint catalogues and cross-correlations, Eq. 21</span></blockquote>
  </div>
  <div class="mcol">
    <span class="who">Nagura et al.</span>
    <blockquote>&ldquo;Computing \( {\rm Cov}(\xi_i,\xi_j) \) directly requires \( O(\ell_{\rm max}^{6}) \) operations and is therefore computationally intractable, since we are interested in angular scales up to \( \ell_{\rm max}\sim \) a few \( \times\,O(10^3) \) (or even \( O(10^4) \)).&rdquo;<span class="cite">&sect; 2.3, below Eq. 2.8</span></blockquote>
    <blockquote>&ldquo;the iNKA relies on the mode-coupling kernel being sharply localized around \( \ell \simeq \ell' \). It is therefore effective for diagonal-dominated quantities, but its errors can be amplified in situations where off-diagonal information is required with high precision.&rdquo;<span class="cite">&sect; 4.2, on Fig. 5</span></blockquote>
    <blockquote>&ldquo;We further merge galaxies into coarse grid (\( N_{\rm side}=256 \) or \( 512 \) depending on \( \theta_i \)) to reduce the size of the matrices.&rdquo;<span class="cite">&sect; 4.3 &mdash; one field, ten bins, no timings reported</span></blockquote>
  </div>
</div>
```

::: {.notes}
Two papers, three months apart, arriving at the same instruction from opposite directions: stop
modelling the window and count it.

Wolz: the catalogue *is* the mask, so band-limit it at the interparticle scale and split the pairs
— self-pairs exactly, distinct pairs through the narrow-kernel approximation. Exact summation
where the sample is sparse, iNKA where it is dense, and it is in NaMaster today.

Nagura: iNKA is good to ten per cent on the harmonic diagonal and that is not the question, because
the Legendre transform is a double sum that reads every off-diagonal element. Twenty-five to thirty
per cent low on the real-space diagonal at six arcminutes. The fix is to count quadruplets rather
than transform an approximated matrix.

And the thing I would leave you with: quartic counts and Wolz's exact summation are the same object
in different spaces. Both are Wick's theorem evaluated on the actual sources rather than on a
modelled window.
:::
