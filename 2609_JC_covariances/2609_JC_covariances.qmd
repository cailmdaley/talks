...
<div class="two">
  <div>
    <svg class="pA" data-h="470" data-x0="96" data-w="664"
         viewBox="0 0 790 610" width="790" height="610"></svg>
  </div>
  <div>
    <svg class="pK" data-h="190" data-x0="104" data-w="726"
         viewBox="0 0 880 330" width="880" height="330"></svg>
    <svg class="pC" data-h="250" data-x0="104" data-w="726" data-tilde="1"
         viewBox="0 0 880 340" width="880" height="340"></svg>
  </div>
</div>
</div>
```
...
On the left: we observe the field times a window. Note the window is not a box — its edges are
tapered, apodized, and that taper matters as much as the size of the footprint.

On the right, what that does. Multiplying in position space is convolving in harmonic space,
so each observed mode is no longer its own: it is a weighted sum of its neighbours, and the
kernel says how many neighbours. Take the expectation and the smearing becomes a matrix —
the pseudo-spectrum, in red, is the true spectrum seen through the coupling matrix. Watch the
bump get rounded off.
...
five, with sidelobes reaching much further than that. Same area in the last two cases; the
difference is entirely complexity. Everything downstream is a consequence of this number.
:::
