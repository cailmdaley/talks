...
Gravitational lensing distorts galaxy shapes as light traverses the large-scale matter distribution. Correlating those shapes measures the **matter power spectrum** — a direct probe of structure growth under dark matter and dark energy.
...
The key parameter is S₈, which encodes the amplitude of structure growth. The tension between cosmic shear and CMB estimates of S₈ is one of the open questions in the field.
...
This release turns one blinded weak-lensing catalogue into 2D cosmic-shear constraints in configuration and harmonic space.
:::
...
Paper III is the gatekeeper: it decides which catalogue version and which scales are clean enough for cosmology.
:::
...
::: notes
Paper IV is the configuration-space cosmology result.
:::
...
::: notes
Paper V is the harmonic-space cosmology result and robustness study.
Paper IV and Paper V then run two cosmology pipelines on the same blinded inputs.
:::
...
2D PTE scan used to define the fiducial angular cuts.
:::
...
The top plot shows the direct pure-mode decomposition, where the residual problem sits at the CCD scale.
The lower plot shows how that turns into an objective scale-cut choice.
...
::: {style="line-height:1.6em;font-size:0.92em"}
- No tomographic bins yet: this release uses a single 2D source distribution $n(z)$
- $n(z)$ is estimated with a SOM-based colour--redshift calibration using CFHTLenS W3 spectroscopy
- Both analyses marginalize over a mean redshift shift $\Delta z$
- Full multi-bin photo-$z$ calibration is the main prerequisite for tomographic UNIONS shear cosmology
:::

::: notes
This release is 2D because the full multi-bin photo-z calibration is not ready yet.
The redshift distribution is still carefully estimated and carried into both analyses with a Delta-z nuisance parameter.
:::
...
::: {style="line-height:1.6em;font-size:0.92em"}
- **Configuration space:** analytical two-point covariance, cross-checked against jackknife and GLASS mock validation
- **Harmonic space:** iNKA / OneCovariance-style analytical covariance with robustness tests
- Blind-to-blind covariance differences are driven only by the shifted $n(z)$ and remain small
- Covariance choices are propagated as robustness tests rather than tuned after the fact
:::

::: notes
The two analyses do not share exactly the same covariance machinery, but both are analytical predictions validated against additional checks.
:::


## Likelihoods and systematics

::: {style="line-height:1.6em;font-size:0.92em"}
- Common cosmology model, common $n(z)$ inputs, common scale cuts from Paper III
- Separate likelihood implementations in configuration and harmonic space
- Nuisance treatment includes intrinsic alignments, baryons / nonlinear modelling, and $\Delta z$
- The comparison between the two pipelines is part of the robustness story, not an afterthought
:::

::: notes
This is the final setup before showing contours: same scientific assumptions, different analysis basis.
:::

## Cosmological contours

<!-- TODO: replace with unblinded contours after Monday 2026-03-16 -->

:::::::::::::: {.columns}
::: {.column style="width:60%"}

![](images_local/contours_s8_omegam_config_harm_planck.png){width=84% fig-align="center"}
...
- Compact view of the first UNIONS 2D cosmic-shear constraints
- Configuration and harmonic space are broadly consistent with each other
- Error bars remain large because this release is non-tomographic

::: {style="font-size:0.9em;margin-top:30px;color:crimson"}
**Placeholder (Blind A / current working figure).** Final unblinded contours to be inserted.
:::
...
::: notes
This is the compact cosmology view I actually want people to remember.
Configuration and harmonic space are both there, along with Planck, in one clean panel.
[After unblinding: quote the S₈ value and compare to Planck and other shear surveys.]
:::


## $S_8$ comparison

<!-- TODO: replace with unblinded whisker plot after Monday 2026-03-16 -->

:::::::::::::: {.columns}
::: {.column style="width:60%"}

![](images_local/contours_stage3_surveys_current.png){width=96% fig-align="center"}
...
- Harmonic and configuration-space UNIONS contours shown against DES Y3, KiDS-1000, and HSC Y3
- Broad consistency with other Stage-III weak-lensing surveys
- Current release is less constraining because it is intentionally conservative and non-tomographic

::: {style="font-size:0.85em;margin-top:20px;color:crimson"}
**Placeholder (Blind A / current working figure).**
:::
...
::: notes
This is the external comparison view.
It situates UNIONS among existing Stage-III shear surveys without pretending this release is already maximally constraining.
[After unblinding: decide whether the whisker plot belongs in the main deck or backup.]
:::
...
<!-- TODO: update with unblinded contours -->

![](images_local/contours_weak_lensing_surveys.png){width=55% fig-align="center"}

::: {style="font-size:0.8em;text-align:center"}
$\Omega_m$–$S_8$ contours: UNIONS (harmonic + config-space) compared to DES Y3, KiDS-1000, HSC Y3, and DES+KiDS. Blind A shown.
:::
