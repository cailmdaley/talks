...
<div class="pick"></div>
<div class="mrow">
  <div class="mcell"><canvas style="width:380px;height:380px"></canvas>
    <div class="mlab"><b>exact</b>direct sum over modes</div></div>
  <div class="mcell fragment" data-fragment-index="0"><canvas style="width:380px;height:380px"></canvas>
    <div class="mlab"><b>NKA</b>one mask-product kernel</div></div>
  <div class="eqcol fragment" data-fragment-index="0">
    <div>
      <span class="nm">narrow-kernel approximation</span>
      <span class="eq">\( {\rm Cov}\big(\tilde C(k),\tilde C(k')\big)\;\simeq\;2\,\bar C_{kk'}^{\,2}\;\Xi_{kk'} \)</span>
    </div>
    <div>
      <span class="nm">C at the midpoint</span>
      <span class="eq">\( \bar C_{kk'}\;\equiv\;\tfrac12\big[C(k)+C(k')\big] \)</span>
    </div>
    <div>
      <span class="nm">coupling kernel</span>
      <span class="eq">\( \Xi_{kk'}\;=\;\big|W^{(2)}_{k-k'}\big|^{2} \)</span>
      <span class="eq">\( W^{(2)}_{k}\;\equiv\;\textstyle\sum_x w^2(x)\,e^{-ikx} \)</span>
    </div>
  </div>
  <div class="mcell fragment" data-fragment-index="1"><canvas style="width:380px;height:380px"></canvas>
...
coupling kernel from three slides ago, the mask's own power at mode separation k minus k-prime
...
<div class="eqs"></div>
<svg class="pP" data-h="80" data-foot-at="2" viewBox="0 0 1730 238" width="1730" height="238"></svg>
<div class="two lower">
  <div class="toycol fragment" data-fragment-index="0">
    <svg class="pMps" data-h="342" data-foot-at="2" data-x0="132" data-w="352"
         viewBox="0 0 520 434" width="520" height="434"></svg>
    <div class="eqblock nwdef">
      <span class="nm">mask shot noise</span>
      <span class="eq">\( \tilde N_w=\tfrac{1}{4\pi}\sum_i w_i^2 \)</span>
    </div>
  </div>
  <div class="figblock fragment" data-fragment-index="2">
...
<div class="fragment" data-fragment-index="1" style="height:0"></div>
</div>
```
...
Now drop a real footprint on it — edges, and a couple of holes. Sources inside the gaps simply do not
...
<div class="two lower">
  <div class="toycol wide">
    <svg class="pMps" data-h="300" data-foot-at="0" data-cloud="1" data-pixel="1"
         data-x0="132" data-w="820" viewBox="0 0 990 392" width="990" height="392"></svg>
  </div>
  <div class="cloudcol">
    <div class="eqblock">
...
<div class="eqs"></div>
<div class="saywhat fragment" data-fragment-index="0" data-own="1">zero-separation information has no scale — its spectrum is flat, so the white-noise machinery handles it exactly.</div>
...
w-bar. Write that out for a catalogue against itself and you get two sums: one over distinct
pairs, i not equal to j, which is a perfectly ordinary object, and one over every source paired
with itself. That second sum does not go away when you smooth. It is the diagonal of the product,
and it is there in every catalogue auto-spectrum.

The good news is what that term looks like once you transform it.
...
<div class="eqs"></div>
<svg class="pPair" data-h="150" viewBox="0 0 1730 262" width="1730" height="262"></svg>
...
Pick a separation. Every pair of sources at that separation — and there are hundreds of them,
that web is all of them — contributes the product of its two values. Add those up, divide by how
...
<div class="nka">
  <div class="corr big">
    <div>
      <span class="nm">a linear map in, so the covariance is a <i>double</i> sum</span>
...
::: {.notes}
So: xi is a linear transform of C-tilde. Linear transform in means the covariance propagates by
sandwiching — and that is a *double* sum, over ell and ell-prime. Every off-diagonal element of
the harmonic covariance contributes, weighted by two oscillating Legendre kernels. There is no
way to get the real-space covariance right while getting the harmonic off-diagonals wrong.

And the exact harmonic covariance is order ell-max to the sixth, which the paper calls
intractable at six thousand. So you are forced to approximate, and that is where the trouble
enters.

Three things people actually use. F-sky, the Knox formula: strictly diagonal, inflated by one
over f-sky, pretending the mask deletes modes without coupling them. iNKA, which we spent the
first half of the talk on. And the one that is going to win: weighted quartic counts, from
Schneider and from Shirasaki. Take every pair in bin i and every pair in bin j, and the Gaussian
four-point function factorises into two cross-pairings, each the theory xi at the actual
separations. Notice what is absent: there is no window function anywhere. No coupling matrix, no
f-sky, no assumption about the kernel — because the sum runs over real positions, so the
footprint, the holes, the boundaries and the weights are all encoded exactly by which
quadruplets exist. The window is not modelled. It is counted.
:::
