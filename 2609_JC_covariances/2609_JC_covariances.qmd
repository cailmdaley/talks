  ...
  <div class="methods">
    ...
    <div class="mrow2 apart fragment">
      <div class="mtxt"><div class="mbox">
        ...
        <span class="eq">\( {\rm Cov}(\xi_i,\xi_j)=\big(N_{{\rm pair},i}N_{{\rm pair},j}\big)^{-1}\textstyle\sum_{abcd} w_aw_bw_cw_d\,\Delta_i(\theta_{ab})\,\Delta_j(\theta_{cd})\cdot\tfrac12\big[\xi(\theta_{ac})\xi(\theta_{bd})+\xi(\theta_{ad})\xi(\theta_{bc})\big] \)</span>
        </div>
      </div>
...
<div class="figpair">
  ...
  <div class="papercap"><span class="src">Nagura et al., arXiv:2605.16186 — Fig. 5</span>Diagonal components of \(\mathrm{Cov}(\xi[\theta_-,\theta_+],\xi[\theta'_-,\theta'_+])\), measured from Gaussian realizations (dashed line) and calculated using analytical methods (solid lines). Note that we consider XMM field only for this figure.</div>
    <div class="punchbox">
      <span class="nm">iNKA vs simulations</span>
      <span class="pv">10% in harmonic space</span>
      <span class="par">&darr;</span>
      <span class="pv hot">25&ndash;30% in \( \xi(\theta) \)</span>
    </div>
  </div>
</div>
</div>
