...
<div class="two split">
  <div class="splitfig">
    <svg class="pSplit" data-h="270" data-x0="96" data-w="880"
         viewBox="0 0 1000 366" width="1000" height="366"></svg>
  </div>
  <div class="toystack fragment" data-fragment-index="2">
    <svg class="pXi" data-h="124" data-x0="76" data-w="452" viewBox="0 0 560 214" width="560" height="214"></svg>
    <svg class="pWhite" data-h="124" data-x0="76" data-w="452" viewBox="0 0 560 214" width="560" height="214"></svg>
  </div>
</div>

<div class="corr big tight">
  <div class="fragment" data-fragment-index="3">
    <span class="nm noise">self-pair field, and its transform</span>
    <span class="eq">\( n(x)\;\equiv\;\langle a^{2}\rangle\,\textstyle\sum_i w_i^{2}\,G^{2}(x-x_i),\qquad \langle a^{2}\rangle=\textstyle\sum_\ell C_\ell,\qquad N_{\ell-\ell'}=\textstyle\sum_x n(x)\,e^{-i(\ell-\ell')x} \)</span>
  </div>
  <div class="fragment" data-fragment-index="3">
...
variance of the field at a single point. And look at the phase on that second term: it depends
only on ell minus ell-prime. Not on ell and ell-prime separately. That is the algebraic signature
of a statistically homogeneous noise field, and it does not go away when you smooth the mask.

The two panels on the right are why "noise" is the right word rather than a metaphor. Split every
pair of sources by separation. The distinct pairs sit at finite separation and trace the
correlation function. The self-pairs all sit at zero separation, with height equal to the variance
at a point, which is the integral of the whole power spectrum. Transform that. A spike at zero
lag is a flat line at every multipole. So the self-pair term behaves exactly like white noise —
and white noise is the one thing this machinery already handles exactly.

Now the picture on the left, which is the same split in position space. Blue is the band-limited
mask from the previous slide. Square it, because the covariance kernel wants the mask product,
and the square separates into two stacked bands: the red floor is every source paired with
itself, and the blue bulk above it is every distinct pair. The red band tracks the source density
— it is largest where sources are sparse relative to the cloud size — and it is there for a
perfectly noiseless field.

So, first line underneath. Name the self-pair piece: it is a local variance map, the variance at a
point times the self-pair half of the mask product, and its transform is the N that appears in the
covariance. That is the connection — the noise in the covariance is literally the harmonic
...
<div class="r-stack figswap">
  ...
  <div class="figone fragment fade-in" data-fragment-index="0">
