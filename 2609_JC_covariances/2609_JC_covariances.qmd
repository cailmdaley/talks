...
```{=html}
<div class="nka" data-frag-step="5">
  <div class="corr big">
    <div>
      <span class="nm">a linear map in, so the covariance is a <i>double</i> sum</span>
      <span class="eq">\( {\rm Cov}(\xi_i,\xi_j)\;\propto\;\sum_{\ell}\sum_{\ell'}\big[P_{\ell+1}-P_{\ell-1}\big]_i\,\big[P_{\ell'+1}-P_{\ell'-1}\big]_j\;{\rm Cov}\big(\tilde C_\ell,\tilde C_{\ell'}\big) \)
        <span class="tag">\( \mathcal{O}(\ell_{\rm max}^{6}) \)</span></span>
    </div>
  </div>
  <div class="menu wide">
    <div class="fragment">
    ...
    <div class="fragment">
      ...
      <span class="note">no \(\ell\) anywhere: the window is <b>counted</b>, not modelled.</span>
    </div>
  </div>
</div>
```
