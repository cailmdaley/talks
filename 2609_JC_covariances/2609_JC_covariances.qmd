...
## It is a sum of modes {.nka-slide}

```{=html}
<div class="nka" data-step="1">
<div class="eqs"></div>
<svg class="pA" data-h="180" viewBox="0 0 1730 270" width="1730" height="270"></svg>
<svg class="pM" data-rowh="130" viewBox="0 0 1730 430" width="1730" height="430"></svg>
</div>
```

::: {.notes}
Decompose it. Each Fourier mode is one wavenumber — one wavelength, one amplitude, one phase — and adding them all back up returns the field exactly. These three are actual components of the realisation above, not cartoons.
:::

## The spectrum is what's left {.nka-slide}

```{=html}
<div class="nka" data-step="2">
<div class="eqs"></div>
<svg class="pM" data-rowh="100" viewBox="0 0 1730 330" width="1730" height="330"></svg>
<svg class="pC" data-h="300" data-marks="1" viewBox="0 0 1730 400" width="1730" height="400"></svg>
</div>
```

::: {.notes}
Throw away the phases, keep the expected squared amplitude of each mode, and you have the power spectrum. Three modes, three points on the curve. C(k) is the whole statistical content of a Gaussian field, and it is what we are trying to measure.
:::

## We only see part of it {.nka-slide}

```{=html}
<div class="nka" data-step="3">
<div class="eqs"></div>
<svg class="pA" data-h="260" viewBox="0 0 1730 350" width="1730" height="350"></svg>
<svg class="pC" data-h="250" viewBox="0 0 1730 340" width="1730" height="340"></svg>
</div>
```

::: {.notes}
Now the mask. We observe the field times a window — and note the window is not a box. Its edges are tapered, apodized, and that taper turns out to matter as much as the size of the footprint.
:::

## Complexity, not size, sets the kernel {.nka-slide}

```{=html}
<div class="nka" data-step="4">
<div class="eqs"></div>
<svg class="pZ" viewBox="0 0 1730 660" width="1730" height="660"></svg>
</div>
```

::: {.notes}
Multiplying in position space is convolving in harmonic space, and the shape of that convolution kernel is set by the mask's complexity, not just its area. Wide and smoothly apodized buys a compact kernel. Small buys a broader one. Holes and hard edges buy a kernel with sidelobes that reach across tens of modes — and that third picture is what a real survey footprint looks like.
:::

## Multiplying by the mask convolves the modes {.nka-slide}

```{=html}
<div class="nka" data-step="5">
<div class="eqs"></div>
<svg class="pA" data-h="200" viewBox="0 0 1730 290" width="1730" height="290"></svg>
<svg class="pC" data-h="290" data-kernel="1" viewBox="0 0 1730 390" width="1730" height="390"></svg>
<div class="pick"></div>
</div>
```

::: {.notes}
So each observed mode is no longer its own. It is a weighted sum of its neighbours, with the weights given by the mask's transform. Switch masks and watch the kernel: this is the object that everything downstream depends on.
:::

## The kernel smears the spectrum {.nka-slide}

```{=html}
<div class="nka" data-step="6">
<div class="eqs"></div>
<svg class="pC" data-h="440" data-kernel="1" data-tilde="1" data-kh="220" viewBox="0 0 1730 560" width="1730" height="560"></svg>
<div class="pick"></div>
</div>
```

::: {.notes}
Take the expectation and the smearing becomes a matrix. The pseudo-spectrum you measure is the true spectrum seen through the coupling matrix M — the bump gets rounded off, the whole curve shifts. That much is exact: M is something you compute, not something you approximate.
:::

## Bandpowers make the coupling invertible {.nka-slide}

```{=html}
<div class="nka" data-step="7">
<div class="eqs"></div>
...
## The error bar needs a four-point sum {.nka-slide}

```{=html}
<div class="nka" data-step="8">
<div class="eqs"></div>
<div class="cost"></div>
<svg class="pC" data-h="420" data-kernel="1" data-kh="210" viewBox="0 0 1730 540" width="1730" height="540"></svg>
<div class="pick"></div>
</div>
```

::: {.notes}
The estimator is exact. Its uncertainty is not. The covariance of a quadratic estimator is a four-point object: for every pair of modes, a sum over a third mode of W times W-star times C. That is a cost that scales as the sixth power of the largest mode, and at survey resolution it is simply not computable.
:::

## The narrow-kernel approximation {.nka-slide}

```{=html}
<div class="nka" data-step="9">
<div class="eqs"></div>
<div class="cost"></div>
<div class="mrow">
  <div class="mcell"><canvas style="width:340px;height:340px"></canvas>
    <div class="mlab"><b>exact</b>direct sum</div></div>
  <div class="mcell"><canvas style="width:340px;height:340px"></canvas>
    <div class="mlab"><b>NKA</b>one mask-product kernel</div></div>
  <div class="mcell"><canvas style="width:340px;height:340px"></canvas>
    <div class="mlab"><b>NKA &minus; exact</b>error</div></div>
</div>
<svg class="pC" data-h="150" data-kernel="1" data-kh="110" data-label="0" data-xlabel="0" viewBox="0 0 1730 250" width="1730" height="250"></svg>
...
::: {.notes}
Here is the trick. If C barely changes across the width of the kernel, evaluate it once at the midpoint of the two modes and pull it outside the sum. What is left collapses into a single kernel — the transform of a product of masks — and the cost drops to the same order as the estimator itself. Where the spectrum is smooth that is nearly exact. Where it has structure on the kernel's scale, it is not, and the error is in the off-diagonals. Switch to the holed mask and the whole band goes wrong. That is the subject of the first two papers.
:::
