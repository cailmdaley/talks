...
```{=html}
<div class="nka takeaways">
  <div class="tk">
    <span class="nm">Wolz et al. &mdash; the catalogue is its own mask</span>
    <ul>
      <li>Shot noise is what aliasing becomes under random sampling; band-limiting at the interparticle scale is choosing the pixel scale at Nyquist &mdash; \( \theta_{\rm c} \) is a choice, defended by a plateau.</li>
      <li>The NKA works best where you would expect it to fail: sparse catalogues are self-pair dominated, and self-pairs are exact &mdash; the approximation is load-bearing in the dense regime.</li>
      <li>The pixel did not vanish; it moved from the estimator into the covariance, as a Gaussian cloud rather than a grid. Catalogue estimators are fast, and in NaMaster v3.</li>
    </ul>
  </div>
  <div class="tk">
    <span class="nm">Nagura et al. &mdash; diagonal accuracy is not enough</span>
    <ul>
      <li>iNKA is ~10% on the \( \ell \) diagonal and unconstrained off it; the Legendre transform sums every off-diagonal \( \rightarrow \) &minus;25&ndash;30% on \( {\rm Cov}(\xi,\xi) \) at 6&prime; &mdash; HSC-Y3, Gaussian sims, no shape noise.</li>
      <li>The control experiment carries the result: project the simulated harmonic covariance and the truth comes back &mdash; transform fine, input wrong. The off-diagonal diagnosis itself is by elimination; the ratio panel is simulation noise.</li>
      <li>Quartic counts track the sims &mdash; but merged onto \( N_{\rm side} \) 256&ndash;512, one field, 10 bins, no cost reported; and HSC's six patches are the hardest case for a narrow kernel.</li>
    </ul>
  </div>
</div>
```
