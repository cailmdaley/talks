...
## A catalogue mask is a set of delta functions {.nka-slide}
...
<div class="two lower">
  <div class="toycol fragment">
    <svg class="pKC" data-h="170" data-x0="86" data-w="400"
         viewBox="0 0 520 262" width="520" height="262"></svg>
    <div class="toyfoot">the coupling kernel, at separation Δk. It never narrows.</div>
  </div>
  <div class="figblock fragment">
    ...
    <div class="figside">
      ...
      <p class="verbatim">Mask pseudo power spectrum for the <i>Quaia</i> survey mask, comparing catalogue- and mask-based estimators. At the catalogue level, the constant shot noise floor (\(\tilde{N}_w\)) is reached at scales far below the mean interparticle scale (\(\theta_{\rm c}\)).</p>
    </div>
  </div>
</div>
</div>
...
## Products of delta functions are zero or infinite {.nka-slide}

```{=html}
<div class="nka" data-step="7" data-ledger-from="7">
<svg class="pProd" data-x0="90" data-w="1560" viewBox="0 0 1730 268" width="1730" height="268"></svg>
<svg class="pCloud" data-h="115" data-x0="90" data-w="1560" viewBox="0 0 1730 200" width="1730" height="200"></svg>
<div class="ctl-slider inline">
  <label>Gaussian cloud <input type="range" data-var="thc" min="0" max="420" value="0"></label>
</div>
</div>
```
...
everywhere. And one catalogue against itself is worse: every source lands exactly on itself, so
the product is a delta squared, which diverges. Both ways, a dead end — and that sentence is the
whole motivation for the paper.

The fix is to stop pretending you know where a source is to infinite precision. Give each one a
Gaussian cloud of finite size — band-limit the mask — and the products become ordinary finite
numbers. Turn the knob. The sweet spot is around the mean source spacing, which is the
resolution you had all along; it gives the catalogue mask the band-limit a pixel mask always
secretly had, chosen at a scale that means something instead of at an arbitrary pixel size.
Note what kind of move this is: θ_c is a choice, not a derivation. They justify it by showing
the answer plateaus over a wide range of it.
:::
...
<div class="eqs"></div>
<div class="saywhat" data-own="1">zero-separation information has no scale — its spectrum is flat, so the white-noise machinery handles it exactly.</div>
<div class="two">
  <div><svg class="pXi" data-h="200" data-x0="90" data-w="700" viewBox="0 0 840 300" width="840" height="300"></svg></div>
  <div><svg class="pWhite" data-h="200" data-x0="90" data-w="700" viewBox="0 0 840 300" width="840" height="300"></svg></div>
</div>
<div class="corr">
  <div class="fragment">
    <span class="nm">correction 1 — band-limit the mask</span>
    <span class="eq">\( w(x)=\sum_i w_i\,\delta(x-x_i) \;\longrightarrow\; \bar w(x)=\sum_i w_i\,G_{\theta_{\rm c}}(x-x_i) \)</span>
    <span class="note">every source becomes a cloud, so mask products are finite.</span>
  </div>
  <div class="fragment">
    <span class="nm">correction 2 — take the self-pairs back out</span>
    <span class="eq">\( \mathcal{W}_{ac}\;\equiv\;\bar w_a\bar w_c-\delta_{ac}\,\overline{w_a^{2}} \)</span>
    <span class="note">leave the self-pairs in and they are counted twice. \(\overline{w^2}\) is its own catalogue field: the transform of the <i>squared</i> cloud, not the square of the transform.</span>
  </div>
  <div class="fragment">
    <span class="nm">correction 3 — and back in, exactly</span>
    <span class="eq">\( {\rm Cov}\;\simeq\;\underbrace{\bar C\,\bar C\;\Xi\big(\mathcal{W},\mathcal{W}\big)}_{\rm sig\times sig}\;+\;\underbrace{2\,\bar C\;\Xi\big(\mathcal{W},\,\overline{(aw)^2}\big)}_{\rm sig\times noise}\;+\;\underbrace{\Xi\big(\overline{(aw)^2},\,\overline{(aw)^2}\big)}_{\rm noise\times noise} \)</span>
    <span class="note">the approximation carries only the distinct pairs; everything involving a self-pair is exact.</span>
  </div>
</div>
...
## Noise-dominated: the exact term carries it {.nka-slide}
...
## Signal-dominated: the approximation carries it {.nka-slide}
...
<div class="nka">
  <div class="corr">
    <div>
      <span class="nm">the correlation function is a linear map of the <i>windowed</i> spectrum</span>
      <span class="eq">\( \xi_i = \frac{n^2}{N_{{\rm pair},i}}\;2\pi\sum_\ell \big[P_{\ell+1}-P_{\ell-1}\big]_i\;\tilde C_\ell \)</span>
      <span class="note">a pair count is unbiased by the mask — the geometry enters as the normalisation, through the pairs that actually exist.</span>
    </div>
    <div>
      <span class="nm">so its covariance is a <i>double</i> sum</span>
      <span class="eq">\( {\rm Cov}(\xi_i,\xi_j)\;\propto\;\sum_{\ell}\sum_{\ell'}\big[P_{\ell+1}-P_{\ell-1}\big]_i\,\big[P_{\ell'+1}-P_{\ell'-1}\big]_j\;{\rm Cov}\big(\tilde C_\ell,\tilde C_{\ell'}\big) \)</span>
      <span class="note">every off-diagonal element contributes — and the exact \({\rm Cov}(\tilde C_\ell,\tilde C_{\ell'})\) costs \(\mathcal{O}(\ell_{\rm max}^{6})\), intractable at \(\ell_{\rm max}\simeq6000\). An approximation is forced.</span>
    </div>
  </div>
  <div class="menu">
    <div class="fragment">
      <span class="nm">f<sub>sky</sub> — Knox</span>
      <span class="eq">\( \delta_{\ell\ell'}\dfrac{2C_\ell^2}{(2\ell+1)f_{\rm sky}} \)</span>
      <span class="note">pretend the mask deletes modes but never couples them: diagonal, divided by the sky fraction.</span>
    </div>
    <div class="fragment">
      <span class="nm">iNKA — the one we know</span>
      <span class="eq">\( \tfrac12\Big(\dfrac{\tilde C_\ell+\tilde C_{\ell'}}{\langle w^2\rangle}\Big)^{2}\,\Xi_{\ell\ell'}(w^2,w^2) \)</span>
      <span class="note">pull the signal out of the coupling sum; what is left is pure geometry, built from the squared mask.</span>
    </div>
    <div class="fragment">
      <span class="nm">quartic counts — Schneider, Shirasaki</span>
      <span class="eq">\( \sum_{\rm pairs\;of\;pairs} w^4\;\dfrac{\xi\xi+\xi\xi}{2} \)</span>
      <span class="note">Wick's theorem in configuration space, on the real galaxies. The window is never modelled — it is <b>counted</b>.</span>
    </div>
  </div>
</div>
```
...
::: {.notes}
Second paper. Nagura, Terasawa, Terawaki and Takada, from Kavli IPMU and Chiba. Same
machinery, one step further: what happens when you take a harmonic-space covariance and
go to real space.

Top line, and it is the structural fact the paper leans on, from Friedrich's appendix C:
xi is a linear transform of the *windowed* spectrum. A pair count is not biased by the
mask — but its harmonic representation is in terms of C-tilde, and the normalisation is
the number of pairs that actually exist in the footprint, not an idealised area. So the
geometry enters twice.

Second line is the load-bearing equation. Linear transform in, so the covariance
propagates by sandwiching — and that is a *double* sum over ell and ell-prime. Every
off-diagonal element of the harmonic covariance contributes, weighted by two oscillating
Legendre kernels. And the exact harmonic covariance is order ell-max to the sixth, which
the paper calls intractable at six thousand. So you are forced to approximate, and that
is where the trouble enters.

Three things people actually use. F-sky, the Knox formula: strictly diagonal, inflated by
one over f-sky, pretending the mask deletes modes without coupling them. iNKA, which we
spent the first half of the talk on. And the one that is going to win: weighted quartic
counts, from Schneider and from Shirasaki. Take every pair in bin i and every pair in bin
j, and the Gaussian four-point function factorises into two cross-pairings, each the
theory xi at the actual separations. Notice what is absent: there is no window function
anywhere. No coupling matrix, no f-sky, no assumption about the kernel — because the sum
runs over real positions, so the footprint, the holes, the boundaries and the weights are
all encoded exactly by which quadruplets exist. The window is not modelled. It is counted.
:::
