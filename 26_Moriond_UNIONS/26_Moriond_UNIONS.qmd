...
- **Intrinsic alignments** — $A_\mathrm{IA}$ degenerate with $S_8$ in one bin; Gaussian prior from red/blue split ($0.83 \pm 0.7$)

- **Covariance** — analytical (CosmoCov, iNKA); validated with jackknife and 350 GLASS mocks

- All nuisance priors inflated to be conservative: $\Delta z$ by ${\times}1.4$, $A_\mathrm{IA}$ by ${\times}2$, $m$ by ${\times}3$
...
Finally, we inflate all nuisance priors to be conservative — 1.4 times on delta-z, doubled on A_IA, tripled on m from the measured values. These will all tighten with future work.
:::
