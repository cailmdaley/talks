...
- Synthesize null tests from $\xi_\pm^B$, COSEBIs, and $C_\ell$s to inform scale cuts and sample selection
...
- $\xi_\pm(\theta)$ with PSF leakage parameters ($\alpha$, $\beta$) inferred from $\rho$/$\tau$ diagnostics<br>— uncertainty propagates into the error budget
...
- Same catalog, $n(z)$, and blinding — different basis and weighting of scales
...
## Conservative priors on $n(z)$ and shear bias — both will improve

::: {style="line-height:1.8em"}

- **Redshift distribution** — $r$-band only, so no colors; cross-match with CFHTLenS for $ugriz$, calibrate with SOM + spectroscopic sample. Marginalize over $\Delta z$ with $\mathcal{N}(0.033, 0.018)$

- **Shear multiplicative bias** — MetaCalibration corrects most; residual from blending calibrated with image simulations (Paper II). Marginalize over $m$ with $\mathcal{N}(-0.057, 0.014)$

- We inflate uncertainties to be conservative: $\Delta z$ by ${\times}1.4$, $A_\mathrm{IA}$ by ${\times}2$, $m$ by ${\times}3$ from measured values

:::

::: notes
Two important inputs to the inference that deserve a moment.

First, redshifts. We only use the r-band for shape measurement — that's the CFIS r-band with 0.7 arcsecond seeing. Because we're only measuring shapes in one band, we don't have colors for the full 61-million-galaxy sample. So how do we get a redshift distribution?

We use a small overlap region: the CFHTLenS W3 field, which is 44 square degrees with deep ugriz photometry from five bands. Essentially all our detected sources have CFHTLenS counterparts in that overlap. We cross-match and adopt their ugriz magnitudes.

Then we train a self-organizing map — a SOM — on a spectroscopic sample of about 65,000 galaxies from DEEP2, VVDS, and VIPERS. These are deep spectroscopic surveys that were all observed with the same CFHTLenS filter set, so the colors are directly comparable. The SOM tiles the color-magnitude space, and we assign each UNIONS galaxy to its best-matching cell based on its ugriz photometry. The redshift distribution falls out from the spectroscopic galaxies in each cell, weighted to account for selection biases.

We validate this by building mock catalogs from the MICE2 simulation, running them through the same SOM pipeline, and comparing the recovered n(z) to the true redshifts. This gives us a calibration bias of delta-z = 0.033 — the SOM slightly overestimates the mean redshift. The uncertainty of plus or minus 0.018 comes from jackknife resampling of the spectroscopic calibration sample. We marginalize over this shift as a nuisance parameter in the inference.

The key limitation: we're relying on a 44 square degree overlap to represent 3500 square degrees. Multi-band UNIONS photometry is being assembled across the full footprint, which will give us proper photometric redshifts and enable tomographic bins.

Second, shear calibration. MetaCalibration corrects most of the multiplicative bias by applying known shears to galaxy images and measuring the response. But it can't fully capture the effect of blending — when galaxies overlap, the shape measurement is biased in ways that MetaCalibration doesn't model. Fabian's image simulations in Paper II estimate this residual. The prior on m is uniform from about minus 6 to plus 1 percent — it's not Gaussian because the systematic is one-sided (blending tends to bias shapes in one direction).

Neither parameter — delta-z nor m — is constrained by the data. The posteriors just follow the priors. That's fine for a non-tomographic analysis: we're marginalizing over the uncertainty honestly. Both will tighten with future work: full-footprint photo-z for redshifts, and improved image simulations for shear calibration.
:::
...
- Blind by shifting the mean of $n(z)$: three realizations A/B/C

- We froze catalog version, scale cuts, and robustness criteria before reveal

:::
::: {.column style="width:50%"}

![](images/blind_nz_ABC.png){width=88% fig-align="center"}

::: {style="font-size:0.75em;text-align:center;margin-top:-8px;margin-bottom:12px"}
...
Caveat: the cosmology-paper authors were inadvertently partially unblinded ~2 weeks before the ceremony. No analysis choices were changed after this point.
:::
...
I should be transparent: the authors of Papers IV and V were inadvertently partially unblinded about two weeks before the ceremony. The maximum likelihood estimate — which isn't penalized by priors — peaked at the same S8 for all three blinds, because the S8–A_IA degeneracy allowed A_IA to compensate the n(z) shift. So they could infer which blind was physical. No significant analysis choices were changed after this point — catalog version, scale cuts, and robustness criteria were already frozen.
:::
...
- Not config vs harmonic: COSEBIs from $\xi_\pm$ and from $C_\ell$ both fail
- Filter functions set the sensitivity — COSEBIs concentrate it on contaminated scales
- Residual systematics are present; mitigated through sample selection and scale cuts
...
## Joint B-mode passage constrains scales more tightly than any single test

:::::::::::::: {.columns}
::: {.column style="width:55%"}

- Three estimators weight scales differently — different sensitivity to the same contamination
- Cuts chosen jointly from B-mode PTEs and blinded inference stability (Papers IV/V)
...
COSEBIS $B_n$ PTE scan. Square: adopted cuts.
:::
...
## PSF uncertainty propagates into the cosmological error budget

::: {style="line-height:1.8em"}

- **PSF leakage** — $\alpha$, $\beta$ inferred from $\rho$/$\tau$ diagnostics, jointly fit with cosmology (Paper IV)

- **Covariance** — analytical (CosmoCov, iNKA); validated with jackknife and 350 GLASS mocks

- **Intrinsic alignments** — $A_\mathrm{IA}$ degenerate with $S_8$ in one bin; conservative Gaussian prior from red/blue split ($0.83 \pm 0.7$)

- **Likelihoods** — common model and $n(z)$, separate codes; marginalize over baryonic feedback, $\Delta z$, $m$

:::

::: notes
I want to highlight two things here. First, the PSF leakage treatment in Paper IV: rather than correcting at the catalog level, the leakage parameters are inferred from rho and tau statistics and carried into the cosmological likelihood as Gaussian priors. The PSF model uncertainty propagates into the error budget.
Second, the intrinsic alignment situation. With one redshift bin, A_IA and S8 are almost perfectly degenerate. We can't fit A_IA from the data — so we build a prior from external measurements. This is the main caveat of the non-tomographic analysis: if the prior is wrong, S8 moves. Tomography breaks this because different redshift bins see different ratios of lensing to intrinsic alignment.
:::
...
- Conservative priors — uncertainties inflated ${\times}1.4$/${\times}2$/${\times}3$ on $\Delta z$, $A_\mathrm{IA}$, $m$
- Single redshift bin — tomography will tighten constraints substantially
...
![](images/contour_survey_comparison.png){width=95% fig-align="right"}
:::
::: {.fragment fragment-index=0}
![](images/s8_convergence_with_unions.png){width=95% fig-align="right"}
:::
::::
...
- We set the lower scale cut from B-mode validation + $S_8$ stability
...
![](images/contour_ia_treatments.png){height=920px fig-align="center"}
