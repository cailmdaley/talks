...
<div class="corr big">
  <div class="fragment" data-fragment-index="1">
    ...
    <span class="eq">\( \mathcal{W}(x)\;\equiv\;\bar w^{2}(x)\;-\;\textstyle\sum_i w_i^{2}\,G^{2} \)</span>
  </div>
  <div class="fragment" data-fragment-index="2">
    <span class="nm">self-pair field</span>
    <span class="eq">\( n(x)\;\equiv\;\textstyle\sum_i w_i^{2}\,a_i^{2}\,G^{2} \)</span>
  </div>
  <div class="fragment" data-fragment-index="3">
    <span class="nm">catalogue covariance</span>
    <span class="eq">\( {\rm Cov}\big(\tilde C(k),\tilde C(k')\big)\;\simeq\;2\Big[\;\underbrace{\bar C_{kk'}^{\,2}\,\big|\mathcal{W}_{k-k'}\big|^{2}}_{\rm sig\times sig}\;+\;\underbrace{2\,\bar C_{kk'}\,{\rm Re}\big(\mathcal{W}_{k-k'}\,n^{*}_{k-k'}\big)}_{\rm sig\times noise}\;+\;\underbrace{\big|n_{k-k'}\big|^{2}}_{\rm noise\times noise}\;\Big] \)</span>
  </div>
</div>
...
And white noise is the one thing this machinery already handles exactly. So: take the self-pairs
out of the mask product — that is the first line, w-bar squared minus the self-pair sum, and if
you leave them in they are counted twice. Note what that subtracted piece is: it is its own
catalogue field, the transform of the *squared* cloud, not the square of the transform, because
squaring a Gaussian halves its width.

Then put them back, with the data in them. The second line is the same sum with a-i squared
inside it — the local variance map. It is the data's shot-noise floor, the same object as
N-w-tilde on the mask slide with the values put back.

And then the covariance is three terms: signal against signal, signal against noise, noise
against noise. Only the first carries the approximation. Everything with a self-pair in it is
exact.
